...
from agentbridge.domain.enums import ArtifactType, ClaimLevel, TaskState, VerificationStatus
...
class VerificationService:
    ...
    def verify(
        ...
        with UnitOfWork(self.database) as uow:
            ...
            manager.transition(runtime, TaskState.VERIFYING, "VerificationStarted", "checks_dispatched")
            ...
            for item in envelope.acceptance:
                ...
                if verifier is None:
                    ...
                    continue
                result = verifier.check(item, artifacts, run_dir, workspace, envelope.permissions)
                if item.type == "command":
                    ...
                    if log_path.exists():
                        ...
                        result = result.model_copy(update={"artifact_id": evidence.artifact_id})
                results.append(result)
            repo.save_verification_results(runtime.run_id, results)
            passed = bool(results) and all(r.status == VerificationStatus.PASS for r in results)
            report = ClaimReport(
                allowed_claims=["All declared acceptance checks passed"] if passed else [],
                forbidden_claims=[
            ...
            if passed:
                ...
                manager.transition(
                    ...
                    extra={
                        "failed_checks": [
                            r.check_id for r in results if r.status != VerificationStatus.PASS
                        ]
                    },
                )
                manager.transition(
