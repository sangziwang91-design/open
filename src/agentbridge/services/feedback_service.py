...
class FeedbackService:
    ...
    def build(
        ...
        failure_category = next(
            (r.failure_category for r in failed if r.failure_category), None
        )
        ...
        if not evidence:
            evidence = [
                f"No verification results; current state is {runtime.state.value}"
            ]
        return FeedbackEnvelope(
            ...
            new_constraints=["Retry only the failed acceptance node"]
